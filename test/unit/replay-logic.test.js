...
describe('replay helpers', () => {
  ...
  it('appends replay samples and keeps units plus summary metadata in sync', () => {
    let session = createReplaySession({
      unit: 'mph',
      distanceUnit: 'ft',
      startPlace: { label: 'Queens', countryCode: 'us' },
    });
    ...
    expect(session.firstSample).toMatchObject({
      timestampMs: 1000,
      totalDistanceM: 0,
    });
    expect(session.startPlace).toMatchObject({ label: 'Queens' });
  });
  ...
  it('rebases replay distances to the recording start when recording begins mid-trip', () => {
    ...
    expect(
      buildReplayMetricSeries(legacySession, 'speedMs', 'distance').map((point) => point.xValue)
    ).toEqual([0, 80, 180]);
  ...
  it('preserves more than 1200 replay samples when saving and loading the active session', async () => {
    ...
    expect(restoredSession.firstSample).toMatchObject({
      timestampMs: 1000,
      totalDistanceM: 0,
    });
  ...
  it('retains the first replay sample in metadata-only active sessions after chunk persistence', async () => {
    ...
    try {
      ...
      expect(metadataOnlySession.firstSample).toMatchObject({
        timestampMs: 1000,
        latitude: 40.7128,
        longitude: -74.006,
        totalDistanceM: 0,
      });
      expect(metadataOnlySession.lastSample).toMatchObject({
  ...
  it('falls back to the saved recordings library when there is no active session', async () => {
    await saveReplayLibrary([
      appendReplaySample(
        appendReplaySample(
          createReplaySession({ id: 'library-only' }),
          createSample({ timestampMs: 5000 })
        ),
  ...
  it('archives finalized sessions into a bounded replay library and migrates the legacy slot', async () => {
    ...
    await saveLastReplaySession(
      appendReplaySample(
        appendReplaySample(
          createReplaySession({ id: 'legacy-last' }),
          createSample({ timestampMs: 7000 })
        ),
  ...
  it('removes saved recordings without affecting the rest of the library', async () => {
    await saveReplayLibrary([
      appendReplaySample(
        appendReplaySample(
          createReplaySession({ id: 'keep-me' }),
          createSample({ timestampMs: 1000 })
        ),
        createSample({ timestampMs: 2000, speedMs: 7 })
      ),
      appendReplaySample(
        appendReplaySample(
          createReplaySession({ id: 'delete-me' }),
          createSample({ timestampMs: 3000 })
        ),
  ...
  it('builds summary totals and highlight moments from the replay session', () => {
    ...
    expect(summary).toMatchObject({
      ...
      minAltitudeM: 10,
      maxAltitudeM: 22,
      routeLabel: '6312 Hilltop Ct Fort Lee NJ -> 123 Anderson Ave',
    });
