...
describe('trip-cost storage', () => {
  ...
  it('persists and normalizes trip cost settings', () => {
    ...
    localStorage.setItem(
      'energy_trip_cost_settings_v1',
      JSON.stringify({ unit: 'bad', mode: 'bad' })
    );
    expect(storage.loadTripCostSettings()).toEqual(storage.DEFAULT_SETTINGS);
  });

  it('uses the regional bootstrap distance unit when no trip settings are stored yet', async () => {
    localStorage.setItem(
    ...
    expect(storage.loadTripCostSettings()).toEqual({
      unit: 'mi',
      mode: 'simple',
    });
  });
  ...
  it('merges stored values with defaults', () => {
    ...
    localStorage.setItem(
      'energy_trip_cost_values_v1',
      JSON.stringify({ distance: '75', price: '0.18' })
    );
    expect(storage.loadTripCostValues()).toEqual({
      distance: '75',
      consumption: '',
