import { formatPlaceAddress } from '../shared/route-string.js';
import {
  ACCURACY_WARNING_M,
  MAX_LOG_ROWS,
  SPARKLINE_WINDOW,
...
export function createGpsRateRenderer({ elements, state, t, getLang }) {
  ...
  function buildWarnings(summary) {
    ...
    if (
      (isFiniteNumber(summary.maxIntervalMs) &&
        summary.maxIntervalMs >= SPARSE_INTERVAL_WARNING_MS) ||
  ...
  function renderKpis(summary) {
    ...
    elements.effectiveHzValue.textContent = formatHz(
      latestSample ? latestSample.effectiveHz : null
    );
    ...
    elements.movementValue.textContent = latestSample
      ? getMotionStateLabel(summary.motion.latestState)
      : '—';
  }
  ...
  function renderSummaryCard(summary) {
    ...
    if (!hasSummary) {
      ...
      if (elements.summaryPlaceValue) elements.summaryPlaceValue.textContent = '—';
      return;
    }

    elements.summarySourcePill.textContent =
      summary.source === 'saved' ? t('gpsRateSourceSaved') : t('gpsRateSourceCurrent');
    elements.summarySourcePill.dataset.state =
      summary.source === 'saved' ? 'warning' : getStatusTone();
    elements.summarySavedAt.textContent =
    ...
    elements.summarySpeedFieldValue.textContent = summary.fieldAvailability.speed
      ? t('gpsRateAvailable')
      : t('gpsRateNotSeen');
    elements.summaryHeadingFieldValue.textContent = summary.fieldAvailability.heading
      ? t('gpsRateAvailable')
      : t('gpsRateNotSeen');
    elements.summaryAltitudeFieldValue.textContent = summary.fieldAvailability.altitude
      ? t('gpsRateAvailable')
      : t('gpsRateNotSeen');
    elements.summaryAccuracyValue.textContent = formatMeters(summary.averageAccuracyM);
    if (elements.summaryPlaceValue) {
      elements.summaryPlaceValue.textContent = formatPlaceAddress(summary.place);
    }
  ...
  function renderWarningBadges(summary) {
    ...
    const warnings = summary
      ? summary.warnings
      : [{ kind: 'ok', label: t('gpsRateNoWarnings'), detail: '' }];
  ...
  function renderSparkline() {
    ...
    elements.sparklineRangeLabel.textContent = t('gpsRateLastIntervals', {
      count: SPARKLINE_WINDOW,
    });
  ...
  function renderAvailability(summary) {
    ...
    elements.availabilityHeadingValue.textContent = availabilityText(
      'heading',
      availability.heading
    );
    elements.availabilityAltitudeValue.textContent = availabilityText(
      'altitude',
      availability.altitude
    );
    elements.availabilityAltitudeAccuracyValue.textContent = availabilityText(
      'altitudeAccuracy',
      availability.altitudeAccuracy
    );
    elements.availabilityAccuracyValue.textContent = availabilityText(
      'accuracy',
  ...
  function renderDiagnostics(summary) {
    ...
    elements.jitterValue.textContent =
      summary && isFiniteNumber(summary.jitterMs)
    ...
    elements.missingAltitudeValue.textContent = summary
      ? formatInteger(summary.missingAltitudeCount)
      : '—';
  ...
  function renderLatestSample() {
    ...
    elements.latestLatitudeValue.textContent = latestSample
      ? formatCoordinate(latestSample.latitude)
      : '—';
    elements.latestLongitudeValue.textContent = latestSample
      ? formatCoordinate(latestSample.longitude)
      : '—';
    elements.latestSpeedValue.textContent = latestSample ? formatSpeed(latestSample.speedMps) : '—';
    elements.latestHeadingValue.textContent = latestSample
      ? formatHeading(latestSample.headingDeg)
      : '—';
    elements.latestAccuracyValue.textContent = latestSample
      ? formatMeters(latestSample.accuracyM)
      : '—';
    elements.latestAltitudeValue.textContent = latestSample
      ? formatMeters(latestSample.altitudeM)
      : '—';
    elements.latestAltitudeAccuracyValue.textContent = latestSample
      ? formatMeters(latestSample.altitudeAccuracyM)
      : '—';
    elements.latestGeoTimestampValue.textContent = latestSample
      ? formatLocalTimestamp(latestSample.positionTimestampMs)
      : '—';
    elements.latestPerfTimestampValue.textContent = latestSample
      ? formatPerfTimestamp(latestSample.performanceNowMs)
      : '—';
    elements.latestSampleAgeValue.textContent = latestSample
      ? formatMs(latestSample.sampleAgeMs)
      : '—';
    elements.latestCallbackDeltaValue.textContent = latestSample
      ? formatMs(latestSample.intervalMs)
      : '—';
    elements.latestGeoDeltaValue.textContent = latestSample
      ? formatMs(latestSample.geoTimestampDeltaMs)
      : '—';
  }
  ...
  function renderMotion(summary) {
    ...
    elements.motionSourceValue.textContent = motion
      ? getMotionSourceLabel(motion.latestSource)
      : '—';
    ...
    elements.stationarySamplesValue.textContent = motion
      ? formatInteger(motion.stationarySamples)
      : '—';
  }
  ...
  return {
    ...
    formatHz,
    formatMeters,
    getPlaceLabel: formatPlaceAddress,
  };
}
