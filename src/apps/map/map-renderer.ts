...
import { formatCameraLimitSpeed } from "../../speed/render.js";
import { formatTrapDistance } from "../../speed/traps.js";
...
const ATTRIBUTION_EXPANDED_MS = 5000;
const CAMERA_MAP_DEGRADED_STYLE = {
  version: 8,
  sources: {},
  layers: [{
    id: "camera-map-degraded-background",
    type: "background",
...
function getStatusMessage(status: AnyRecord = {}) {
  ...
  if (status.status === "ready" && Number(status.featureCount) === 0) return t("cameraMapNoCameras");
...
function buildPanel(selectedBasemapId, {
  ...
  const attributionLink = createElement("a", {
    class: "camera-map-attribution-link",
  ...
  const attributionSummary = createElement("summary", {
    class: "camera-map-attribution-summary",
    "aria-label": t("cameraMapAttributionToggle"),
    title: t("cameraMapAttributionToggle"),
    "data-i18n-aria": "cameraMapAttributionToggle",
    "data-i18n-title": "cameraMapAttributionToggle",
  });
  const attribution = createElement("details", {
    class: "camera-map-attribution",
    open: "",
  }, [attributionSummary, attributionLink]);
  ...
  const detailsClose = createElement("button", {
    type: "button",
    class: "camera-map-details-close",
    "aria-label": t("close"),
    "data-i18n-aria": "close",
    text: "×",
  });
  const detailsBody = createElement("div", { class: "camera-map-details-body" });
  const detailsSheet = createElement("aside", {
    class: "camera-map-details-sheet",
    role: "dialog",
    "aria-label": t("cameraMapCameraDetails"),
    "data-i18n-aria": "cameraMapCameraDetails",
  }, [
    createElement("header", {}, [
      createElement("strong", { text: t("cameraMapCameraDetails"), "data-i18n": "cameraMapCameraDetails" }),
      detailsClose,
    ]),
    detailsBody,
  ]);
...
export function createCameraMapWidget(options: AnyRecord = {}) {
  ...
  const restoredBasemapId = normalizeCameraMapBasemapId(initialSessionState?.basemapId);
  let activeBasemap = getCameraMapBasemap(initialSessionState?.basemapId
    ? restoredBasemapId
    : hasUserBasemapPreference
  ...
  const initialNavigationDefaultMode = ["drive", "browse", "auto"].includes(navigationDefaultMode)
    ...
    : "auto";
  let followEnabled = typeof initialSessionState?.followEnabled === "boolean"
    ? initialSessionState.followEnabled
    : loadCameraMapBooleanPreference("follow", false);
  let followPaused = initialSessionState?.followPaused === true;
  let navigationMode = followEnabled ? "drive" : "browse";
  let orientationMode = ["north-up", "heading-up"].includes(initialSessionState?.orientationMode)
    ? initialSessionState.orientationMode
    : loadCameraMapEnumPreference("orientation", ["north-up", "heading-up"], "north-up");
  let projectionMode = ["auto", "flat", "globe"].includes(initialSessionState?.projectionMode)
    ? initialSessionState.projectionMode
    : loadCameraMapEnumPreference("projection", ["auto", "flat", "globe"], "auto");
  ...
  panel.dataset.cameraMapMode = routeMode ? "route" : "window";
  ...
  let attributionCollapseTimer = 0;
  ...
  let presentationMode = ["2d", "3d", "globe"].includes(initialSessionState?.presentationMode)
    ? initialSessionState.presentationMode
    : "2d";
  let buildingsAvailable = false;
  const statusListeners = new Set<(snapshot: AnyRecord) => void>();
  let lastCameraCommand = null;
  let basemapErrorCount = 0;
  let degradedStyleApplied = false;
  ...
  const providers = Array.isArray(overlayProviders)
    ? overlayProviders.filter((provider) => provider && typeof provider === "object")
    : [];

  function notifyOverlayProviders(method, payload = {}) {
    for (const provider of providers) {
      try {
        provider[method]?.({ map, maplibregl, ...payload });
      } catch {
        // A supplemental overlay must never take down the base map.
      }
    }
  }
  ...
  function emitStatus() {
    ...
    const snapshot = getApproachSnapshot();
  ...
  function updateBasemapUi(basemap = activeBasemap) {
    ...
    attributionLink.href = basemap.attributionUrl;
    attributionLink.textContent = t(basemap.attributionKey);
    attributionLink.dataset.i18n = basemap.attributionKey;
    revealAttributionTemporarily();
  }

  function revealAttributionTemporarily() {
    attribution.open = true;
    window.clearTimeout(attributionCollapseTimer);
    attributionCollapseTimer = window.setTimeout(() => {
      attributionCollapseTimer = 0;
      if (!destroyed) attribution.open = false;
    }, ATTRIBUTION_EXPANDED_MS);
  }
  ...
  function waitForRenderableGeometry(timeoutMs = 1800) {
    if (hasRenderableGeometry()) return Promise.resolve(true);
    return new Promise((resolve) => {
      ...
      const check = () => hasRenderableGeometry() && finish(true);
      if (typeof ResizeObserver === "function") {
        observer = new ResizeObserver(check);
        observer.observe(mapEl);
      }
      window.requestAnimationFrame?.(check);
      timeoutId = window.setTimeout(() => finish(hasRenderableGeometry()), timeoutMs);
    });
  }

  function bindWebglContextEvents() {
    cleanupWebglContextEvents();
  ...
  function bindCameraLayerEvents() {
    ...
    map.on("click", CAMERA_POINT_LAYER_ID, (event) => {
      ...
      const popupHtml = buildPopupHtml(popupFeature, {
        includeApproachVisualization: approachLayerEnabled,
        currentDecision: decision,
        approachDetails: resolvedDetails,
      });
      const useDetailsSheet = routeMode
        && typeof globalThis.matchMedia === "function"
        && globalThis.matchMedia("(max-width: 560px)").matches;
      if (useDetailsSheet) {
        detailsBody.innerHTML = popupHtml;
        detailsSheet.hidden = false;
        detailsClose.focus();
        return;
      }
      if (!maplibregl?.Popup) return;
  ...
  function executeNavigationCameraCommand(command) {
    ...
    const movement = {
      ...
      pitch: presentationMode === "3d" ? 52 : (command.pitch ?? 0),
  ...
  function applyProjection() {
    ...
    if (presentationMode === "globe" || projectionMode === "globe") nextProjection = "globe";
    else if (presentationMode === "3d") nextProjection = "mercator";
    ...
    try {
      ...
      setCameraFeatures(currentCameraFeatures, { cache: false });
      addUserPositionLayers();
      applyPresentationLayers();
    } catch {
      // Projection support varies by MapLibre build; navigation remains usable.
    }
  }

  function applyPresentationLayers() {
    if (!map) return;
    const showBuildings = presentationMode === "3d";
    const layers = map.getStyle?.()?.layers || [];
    buildingsAvailable = layers.some((layer) => layer?.type === "fill-extrusion");
    for (const layer of layers) {
      if (layer?.type !== "fill-extrusion" || !layer.id) continue;
      try {
        map.setLayoutProperty?.(layer.id, "visibility", showBuildings ? "visible" : "none");
      } catch {
        // Some raster styles do not expose vector building layers.
      }
    }
    emitStatus();
  }
  ...
  function setPresentationMode(nextMode) {
    if (!["2d", "3d", "globe"].includes(nextMode)) return presentationMode;
    presentationMode = nextMode;
    activeProjection = null;
    applyProjection();
    applyPresentationLayers();
    if (map) {
      const pitch = presentationMode === "3d" ? 52 : 0;
      runProgrammaticCameraMove(() => {
        if (map.easeTo) map.easeTo({ pitch, duration: 320, essential: true });
        else map.jumpTo?.({ pitch });
      });
    }
    emitStatus();
  ...
  function restoreCameraLayersAfterStyle(version, view) {
    ...
    restoreMapView(view);
    applyProjection();
    notifyOverlayProviders("onStyleReady");
    resizeMap();
  }

  async function switchBasemap(nextBasemapId, { persist = true, force = false } = {}) {
    const nextBasemap = getCameraMapBasemap(nextBasemapId);
    if (!force && nextBasemap.id === activeBasemap.id) {
  ...
  async function initMap() {
    ...
    initPromise = (async () => {
      try {
        if (!(await waitForRenderableGeometry())) {
          throw new Error("map-container-has-no-renderable-geometry");
        }
        ...
        const initialView = initialSessionState?.view && Object.keys(initialSessionState.view).length > 0
          ? initialSessionState.view
          : getInitialView({ gpsService, getCurrentPosition });
        ...
        notifyOverlayProviders("onMapReady");

        if (maplibregl.NavigationControl) {
          map.addControl(new maplibregl.NavigationControl({ visualizePitch: false }), "top-right");
        }

        let initialStyleHandled = false;
        const handleInitialStyleReady = () => {
          if (initialStyleHandled) return;
          if (destroyed || !map) {
            resolveReady();
            return;
          }
          initialStyleHandled = true;
          mapReady = true;
          revealAttributionTemporarily();
          addCameraLayers();
          addUserPositionLayers();
        ...
        map.on?.("load", handleInitialStyleReady);
        map.on?.("style.load", handleInitialStyleReady);
        queueMicrotask(() => {
          if (map?.isStyleLoaded?.()) handleInitialStyleReady();
        });

        map.on?.("moveend", queueRefresh);
        map.on?.("moveend", () => notifyOverlayProviders("onViewportChanged", { view: readMapView() }));
        map.on?.("zoomend", () => {
          applyProjection();
          queueRefresh();
        });
        ...
        map.on?.("error", () => {
          ...
          if (basemapErrorCount >= 3) {
            ...
            if (!degradedStyleApplied && typeof map?.setStyle === "function") {
              degradedStyleApplied = true;
              const view = readMapView();
              const version = ++basemapStyleVersion;
              basemapSwitchInProgress = true;
              onceMapEvent("style.load", () => restoreCameraLayersAfterStyle(version, view));
              try {
                map.setStyle(CAMERA_MAP_DEGRADED_STYLE);
              } catch {
                basemapSwitchInProgress = false;
              }
            }
  ...
  function showPanel({ persist = true }: AnyRecord = {}) {
    ...
    if (routeMode) queueMicrotask(initializeVisibleMap);
    else window.setTimeout(initializeVisibleMap, 0);
  }
  ...
  function open(openOptions: AnyRecord = {}) {
    ...
    if (!routeMode) shellManager.openWindow(CAMERA_MAP_WINDOW_ID, { ...openOptions, invokeLifecycle: false });
  }

  function close(closeOptions: AnyRecord = {}) {
    hidePanel(closeOptions);
    if (!routeMode) shellManager.closeWindow(CAMERA_MAP_WINDOW_ID, { ...closeOptions, invokeLifecycle: false });
  }

  function minimize(minimizeOptions: AnyRecord = {}) {
    minimizePanel(minimizeOptions);
    if (!routeMode) shellManager.minimizeWindow(CAMERA_MAP_WINDOW_ID, { ...minimizeOptions, invokeLifecycle: false });
  }

  function restore(restoreOptions: AnyRecord = {}) {
    showPanel(restoreOptions);
    if (!routeMode) shellManager.restoreWindow(CAMERA_MAP_WINDOW_ID, { ...restoreOptions, invokeLifecycle: false });
  }
  ...
  function getSessionState() {
    return {
      ...
      orientationMode,
      projectionMode,
      selectedCameraId: selectedCameraDetails?.cameraId || selectedCameraDetails?.id || null,
    };
  }
  ...
  function destroy() {
    ...
    cleanupWebglContextEvents();
    cleanupWebglContextEvents = () => {};
    notifyOverlayProviders("destroy");
  ...
  return {
    ...
    retry: async () => {
      basemapErrorCount = 0;
      degradedStyleApplied = false;
      if (map?.setStyle) {
        await switchBasemap(activeBasemap.id, { persist: false, force: true });
      }
...
export { createCameraMapWidget as createMapRenderer };
